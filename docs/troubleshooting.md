...
- **Bucket policy missing the CloudFront origin access identity (OAI).** The distribution uses an OAI; without a matching bucket policy S3 blocks the request.
...
2. **Verify the bucket policy includes the OAI canonical user**
   ```bash
   aws s3api get-bucket-policy --bucket $FRONTEND_BUCKET | jq
   ```
   It should match the policy Terraform renders in `infra/terraform/main.tf` under `data "aws_iam_policy_document" "frontend_bucket"`.
...
If the bucket contains the built frontend and the OAI policy is attached, the CloudFront domain should return 200 for both `/` and `/favicon.ico`.
